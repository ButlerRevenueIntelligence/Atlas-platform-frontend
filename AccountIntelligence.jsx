...
async function handleGraphIQSearch(event) {
  ...
  const groupedAccounts = useMemo(() => {
    ...
    (Array.isArray(dashboard?.accounts) ? dashboard.accounts : []).forEach(
      (account) => {
        const name = String(account?.name || "Unnamed account").trim();
        const key = name.toLowerCase();
        map.set(key, {
          id: account?._id || account?.id || key,
          name,
          engagement: 0,
          expansion: 0,
          industry: account?.industry || "Connected Account",
          ...
          revenuePotential: 0,
          revenueLabel: "$0",
          buyingSignal: "Connected account record",
          priority: "Watchlist",
          tone: "Watch",
          dealCount: 0,
          openCount: 0,
    ...
    deals.forEach((deal) => {
      const name = String(deal.company || "Account").trim();
      const key = name.toLowerCase();
      if (!map.has(key)) {
        map.set(key, {
          name,
